...
- $f(n) = g(n) + w * h(n), 0 <= w$
- We can guarantee that the solution found cannot be worse than _w_ times as costly as the optimal solution
...
- __backtracking__ search (DFS)
	...
	- choose the most constrained variable ie. with the fewest legal values
...
- Idea: choose move to position with highest __minmax value__
